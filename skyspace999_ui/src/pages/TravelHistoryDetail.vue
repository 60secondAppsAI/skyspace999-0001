					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateTravelHistory()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2TravelHistorys = true">Add</base-button>
								              <modal :show.sync="modal2TravelHistorys">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Travel History Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="TravelHistoryId" type="text" placeholder="Enter TravelHistoryId" v-model="travelHistoryDetails.travelHistoryId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Travel Date</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<datepicker v-model="travelHistoryDetails.travelDate"  placeholder="Select Date" ></datepicker>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  data() {
    return {
      travelHistoryToAdd: {},
      modal2TravelHistorys: false,
      travelHistoryDetails: null,
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2TravelHistorys = false;
    ...
    async updateTravelHistory() {
      ...
      const jsonData = JSON.stringify(this.travelHistoryDetails);
      const res = await TravelHistoryService.update(jsonData);
...
//        this.getAllTravelHistorys();
    ...
    async getTravelHistoryDetails() {
      try {
			let response = await TravelHistoryService.get(this.travelHistoryId);
			this.travelHistoryDetails = response.data;

        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Seat Assignments</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalSeatAssignments = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalSeatAssignments">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add SeatAssignment</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="SeatAssignmentId" type="text" placeholder="Enter SeatAssignmentId" v-model="seatAssignmentToAdd.seatAssignmentId"></base-input>
  <base-input label="SeatNumber" type="text" placeholder="Enter SeatNumber" v-model="seatAssignmentToAdd.seatNumber"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="seatAssignments" :row-key="record => record.SeatAssignmentId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <SeatAssignmentPictureView :seatAssignments="seatAssignments" />
          </div>
...
import SeatAssignmentPictureView from './SeatAssignmentPictureView.vue';


const seatAssignmentsColumns = [
  "seatAssignmentId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    SeatAssignmentPictureView
  },

  data() {
    return {
      modalSeatAssignments: false,
        isTableView: true,

      columns: [
        {
          title: 'Seat Assignment Id',
		dataIndex: 'seatAssignmentId',
          visible: true,
          scopedSlots: { customRender: 'seatAssignmentId' },
          sorter: true
          //sorter: (a, b) => a.seatAssignmentId - b.seatAssignmentId,
          //sorter: (a, b) => a.seatAssignmentId.localeCompare(b.seatAssignmentId),
        },
        {
          title: 'Seat Number',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} seatAssignments`,
      },
      ...
      sortBy: 'seatAssignmentId',           // Column to sort by
  ...
  methods: {
    ...
    async renderSeatAssignmentsTable() {
      ...
      let seatAssignmentsTableData = [];
      for (let i = 0; i < this.seatAssignments.length; i++) {
        seatAssignmentsTableData.push({
          id: i,
          seatAssignmentId: this.seatAssignments[i].seatAssignmentId,
          year: this.seatAssignments[i].year,
          date: this.seatAssignments[i].date,
          competitionId: this.seatAssignments[i].competitionId,
          seatAssignmentId: this.seatAssignments[i].seatAssignmentId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-seat-assignments',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForSeatAssignmentsChanged', this.searchQuery);
		//this.renderSeatAssignmentsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalSeatAssignments = false;

      const currentDate = new Date().getTime();
      this.seatAssignmentToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.seatAssignmentToAdd);
      console.log(jsonData);
      
      const res = await SeatAssignmentService.addSeatAssignment(jsonData);

        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Delays</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalDelays = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalDelays">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Delay</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="DelayId" type="text" placeholder="Enter DelayId" v-model="delayToAdd.delayId"></base-input>
  <base-input label="Reason" type="text" placeholder="Enter Reason" v-model="delayToAdd.reason"></base-input>
  <base-input label="Duration" type="text" placeholder="Enter Duration" v-model="delayToAdd.duration"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="delays" :row-key="record => record.DelayId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <DelayPictureView :delays="delays" />
          </div>
...
import DelayPictureView from './DelayPictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'delayId',
          visible: true,
          scopedSlots: { customRender: 'delayId' },
          sorter: true
          //sorter: (a, b) => a.delayId - b.delayId,
          //sorter: (a, b) => a.delayId.localeCompare(b.delayId),
        },
        {
          title: 'Reason',
		dataIndex: 'reason',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} delays`,
      },
      ...
      sortBy: 'delayId',           // Column to sort by
  ...
  methods: {
    ...
    async renderDelaysTable() {
      ...
      for (let i = 0; i < this.delays.length; i++) {
        delaysTableData.push({
          id: i,
          delayId: this.delays[i].delayId,
          year: this.delays[i].year,
          date: this.delays[i].date,
          competitionId: this.delays[i].competitionId,
          delayId: this.delays[i].delayId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-delays',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForDelaysChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalDelays = false;

      const currentDate = new Date().getTime();
      this.delayToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.delayToAdd);
      console.log(jsonData);
      
      const res = await DelayService.addDelay(jsonData);
